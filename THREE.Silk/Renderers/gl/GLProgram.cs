...
namespace THREE
{
    ...
    public class GLProgram : DisposableObject, IGLProgram
    {
        ...
        public GLProgram(GLRenderer renderer, string cacheKey, Hashtable parameters, GLBindingStates bindingStates)
        {
            ...
            this.program = (int)renderer.gl.CreateProgram();
            ...
            glVertexShader.SetShaderType(gl,ShaderType.VertexShader, vertexGlsl);
            GLShader glFragmentShader = new GLShader();
            glFragmentShader.SetShaderType(gl,ShaderType.FragmentShader, fragmentGlsl);


            //Debug.WriteLine(glVertexShader.Code);
            //Debug.WriteLine(glFragmentShader.Code);
            gl.AttachShader((uint)program, (uint)glVertexShader.ShaderId);
            GLEnum code1 = gl.GetError();
            gl.AttachShader((uint)program, (uint)glFragmentShader.ShaderId);
            GLEnum code2 = gl.GetError();

            if (parameters["indexOfAttributeName"] != null)
            {
                gl.BindAttribLocation((uint)program, 0, (string)parameters["indexOfAttributeName"]);
            }
            else if ((bool)parameters["morphTargets"] == true)
            {
                gl.BindAttribLocation((uint)program, 0, "position");
            }
            ...
            {
                string programLog = gl.GetProgramInfoLog((uint)program);
                string vertexLog = gl.GetShaderInfoLog((uint)glVertexShader.ShaderId);
                string fragmentLog = gl.GetShaderInfoLog((uint)glFragmentShader.ShaderId);
                ...
                if (linkStatus == 0)
                {
                    ...
                    int validateState;
                    gl.GetProgram((uint)program, GLEnum.ValidateStatus, out validateState);
            ...
            gl.DeleteShader((uint)glVertexShader.ShaderId);
            gl.DeleteShader((uint)glFragmentShader.ShaderId);
        ...
        private string GetShaderErrors(GLShader shader, string type)
        {
            int status;
            GL gl = renderer.gl;
            gl.GetShader((uint)shader.ShaderId, GLEnum.CompileStatus, out status);

            string log = gl.GetShaderInfoLog((uint)shader.ShaderId);
            ...
            gl.GetShaderSource((uint)shader.ShaderId, 4096000, out length, out source);
        ...
        private Hashtable FetchAttributeLocations(int program)
        {
            ...
            int n;
            GL gl = renderer.gl;
            gl.GetProgram((uint)program, GLEnum.ActiveAttributes, out n);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                string name = gl.GetActiveAttrib((uint)program, (uint)i, out size, out attrType);

                int location = gl.GetAttribLocation((uint)program, name);
        ...
        public GLUniforms GetUniforms()
        {
            if (cachedUniforms == null)
            {
                ...
                cachedUniforms.LoadProgram(renderer.gl,(uint)program);
            }
        ...
        public override void Dispose()
        {
            ...
            //{
                renderer.gl.DeleteProgram((uint)program);
                this.program = 0;
            //}
            this.Dispose(true);
